...
----
	@Test
	@Sql({"/test-schema.sql", "/test-message-data.sql"}) // <1>
	void userTest() {
...
----
	@Test
	@Sql("/test-message-data.sql", config = SqlConfig(commentPrefix = "`", separator = "@@")) // <1>
	fun userTest() {
		// run code that relies on the test data
	}
----
...
https://junit.org/junit5/docs/current/message-guide/#running-tests-config-params[JUnit Platform configuration parameter].
...
are loaded: one for `/app-config.xml`, one for `/message-config.xml`, and one for
`{"/message-config.xml", "/order-config.xml"}`. As with the previous example, the
application context loaded from `/app-config.xml` is set as the parent context for the
contexts loaded from `/message-config.xml` and `{"/message-config.xml", "/order-config.xml"}`.
...
----
	...
	@ContextHierarchy({
		...
		@ContextConfiguration(name = "child", locations = "/message-config.xml")
	})
	class BaseTests {}
...
application context for `ExtendedTests` is loaded only from `/test-message-config.xml` and
...
----
	...
	@ContextHierarchy(
		@ContextConfiguration(
			name = "child",
			locations = "/test-message-config.xml",
...
----
	<beans>
		...
		<bean id="loginAction" class="com.example.LoginAction"
				c:username="#{request.getParameter('message')}"
...
`userService`, we are assured that the message service has access to the request-scoped
...
----
	...
	class RequestScopedBeanTests {
		...
		@Test
		void requestScope() {
			request.setParameter("message", "enigma");
...
the message service has access to the session-scoped `userPreferences` for the current
...
----
	...
	class HibernateUserRepositoryTests {
		...
		void createUser() {
			...
			final int count = countRowsInTable("message");
		...
		private void assertNumUsers(int expected) {
			assertEquals("Number of rows in the [message] table.", expected, countRowsInTable("message"));
		}
	}
----
...
----
	...
	class HibernateUserRepositoryTests {
		...
		fun createUser() {
			// track initial state in test database:
			val count = countRowsInTable("message")
...
----
	...
	class DatabaseTests {
		...
		@Test
		@Sql({"/test-schema.sql", "/test-message-data.sql"})
		void userTest() {
			// run code that uses the test schema and test data
		}
	}
...
----
	...
	class TransactionalSqlScriptsTests {
		...
		void assertNumUsers(int expected) {
			assertEquals(expected, countRowsInTable("message"),
...
* Powerful support for link:https://junit.org/junit5/docs/current/message-guide/#extensions-conditions[conditional
...
link:https://junit.org/junit5/docs/current/message-guide/#extensions-parameter-resolution[`ParameterResolver`]
...
Instead, it is a combination of how the message interacts with a web page and how that web
page interacts with other resources. For example, the result of a form view is used as
the input to a message for creating a message. In addition, our form view can potentially
...
* Does our page display a notification to the message to indicate that no results are
...
http://www.gebish.org/manual/current/[The Book of Geb] message's manual.
