...
| Reflects the message logged in at the time of the WebSocket HTTP handshake.
...
to only the message associated with the input message. See <<websocket-stomp-message-destination>>.
...
secure HTTP requests. Typically, a message is authenticated through Spring Security
by using some mechanism such as a login page, HTTP basic authentication, or another way.
The security context for the authenticated message is saved in the HTTP session
...
typically, there is already an authenticated message accessible through
`HttpServletRequest#getUserPrincipal()`. Spring automatically associates that message
with a WebSocket or SockJS session created for them and, subsequently, with all
STOMP messages transported over that session through a message header.

In short, a typical web application needs to do nothing
beyond what it already does for security. The message is authenticated at
...
for that message) and results in a message header being stamped on every `Message` flowing
...
the message is already authenticated at the HTTP transport level, and expects that
the WebSocket or SockJS session contain the authenticated message.
...
that uses a `ChannelInterceptor` to authorize messages based on the message header in them.
...
that ensures the message HTTP session does not expire when the WebSocket session is still active.
...
the message header on the CONNECT `Message`. Spring notes and saves the authenticated
message and associate it with subsequent STOMP messages on the same session. The following
			...
			registration.interceptors(new ChannelInterceptor() {
				...
				public Message<?> preSend(Message<?> message, MessageChannel channel) {
					...
					if (StompCommand.CONNECT.equals(accessor.getCommand())) {
						Authentication message = ... ; // access authentication header(s)
						accessor.setUser(message);
					}
					return message;
				}
			});
...
[[websocket-stomp-message-destination]]
=== User Destinations

An application can send messages that target a specific message, and Spring's STOMP support
recognizes destinations prefixed with `/message/` for this purpose.
For example, a client might subscribe to the `/message/queue/position-updates` destination.
This destination is handled by the `UserDestinationMessageHandler` and
transformed into a destination unique to the message session
...
with other users who subscribe to the same destination so that each message can receive
unique stock position updates.

On the sending side, messages can be sent to a destination such as
pass:q[`/message/{username}/queue/position-updates`], which in turn is translated
by the `UserDestinationMessageHandler` into one or more destinations, one for each
session associated with the message. This lets any component within the application
send messages that target a specific message without necessarily knowing anything more
than their name and the generic destination. This is also supported through an
annotation and a messaging template.

A message-handling method can send messages to the message associated with
...
If the message has more than one session, by default, all of the sessions subscribed
...
NOTE: While message destinations generally imply an authenticated message, it is not strictly required.
A WebSocket session that is not associated with an authenticated message
can subscribe to a message destination. In such cases, the `@SendToUser` annotation
behaves exactly the same as with `broadcast=false` (that is, targeting only the
session that sent the message being handled).

You can send a message to message destinations from any application
...
NOTE: When you use message destinations with an external message broker, you should check the broker
documentation on how to manage inactive queues, so that, when the message session is
over, all unique message queues are removed. For example, RabbitMQ creates auto-delete
queues when you use destinations such as `/exchange/amq.direct/position-updates`.
So, in that case, the client could subscribe to `/message/exchange/amq.direct/position-updates`.
Similarly, ActiveMQ has
https://activemq.apache.org/delete-inactive-destinations.html[configuration options]
for purging inactive destinations.

In a multi-application server scenario, a message destination may remain unresolved because
the message is connected to a different server. In such cases, you can configure a
...
`MessageBrokerRegistry` in Java configuration and the `message-destination-broadcast` attribute
...
connect, including the session ID, message information (if any), and any custom headers the client
