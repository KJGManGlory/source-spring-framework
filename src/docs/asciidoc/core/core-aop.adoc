...
from the join point (message-declared target method) itself but not from an accompanying
...
normal return or exception thrown from the join point (message-declared target method),
...
* If the parameter names have been explicitly specified by the message, the specified
...
conditions (idempotent operations that do not need to go back to the message for conflict
