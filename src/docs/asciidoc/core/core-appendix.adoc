...
-- that is, not usually beans that are important to an end-message but rather beans that do
...
can use autocompletion to let a message choose between several configuration options
...
This works nicely, but it exposes a lot of Spring plumbing to the end message. What we are
