...
----
	...
	RequestEntity<MappingJacksonValue> requestEntity =
		RequestEntity.post(new URI("https://example.com/message")).body(value);
...
you need not use per-message security credentials here. Rather, you can use shared credentials that you define
...
processing task to message-implemented callback interfaces. The JMS template follows the
...
vendors have provided this functionality. Dynamic destinations are created with a message-defined name,
...
that does not tie a message to an EJB container. (See <<jms-receiving-async>> for detailed
...
been exposed as MBeans through an `MBeanExporter`. Any existing message-defined MBeans should
...
returned to the message so that, the next time the method is invoked, the cached result is
