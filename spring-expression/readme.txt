...
- In the resolver/executor model we cache executors.  They are currently recorded in the AST and so if the message chooses to evaluate an expression
...
- when doing arithmetic, eg. 8.4 / 4  and the message asks for an Integer return type - do we silently coerce or
